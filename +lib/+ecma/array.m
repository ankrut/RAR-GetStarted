classdef array < handle
	...
	methods
		...
		% loops
		function obj=forLoop(obj,id,f)
			fWrap = obj.get_fWrap_forLoop(f,id);
...
% 			fWrap = obj.get_fWrap(f);
		...
		function acc = stream(obj,varargin)
			if nargin == 1
				func = @(elm) elm;
			else
				func = varargin{1};
...
% 			fWrap = obj.get_fWrap_forEach(f);
	...
	methods (Access = protected)
		...
		function fWrap=get_fWrap_forLoop(obj,f,id)
			ii = 1;
